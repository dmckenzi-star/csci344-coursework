...
//return an html list item (so that we can add it to an underordered lest)
//return "<li>" + text + "</li>"
 //return `<li>${text}</li>`;
...
const addTodo = text => {
    ...
    const text = inputEl.value //extracts the value the user typed from the DOM element
    //clean it
    const cleanText = cleanTodoText(text);
    //conver HTML
    const htmlText = createTodoItemHTML(cleanText);
    //append it to the unordered list<ul></ul>
